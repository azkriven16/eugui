...
export async function generateMetadata({ params }: { params: Promise<{ name: string }> }) {
  ...
  return {
    title: item ? `${item.title ?? item.name} — eugui` : 'Not found — eugui',
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12">
      <nav className="mb-6 flex items-center gap-2 text-sm text-zinc-500">
        <Link href="/blocks" className="hover:text-zinc-300">
          Blocks
        </Link>
        <span>/</span>
      ...
      <div className="mb-2 flex items-center gap-3">
        ...
        <span className="rounded bg-zinc-800 px-2 py-1 text-xs text-zinc-400">block</span>
      </div>
      {item.description && (
        ...
        <section className="mb-10">
          ...
          <div className="flex flex-wrap gap-2">
            {item.dependencies.map(dep => (
              <span key={dep} className="rounded bg-zinc-800 px-2 py-1 font-mono text-sm text-zinc-300">
                {dep}
              </span>
            ))}
          </div>
